...
def call_structured(
    ...
    kwargs = dict(
        ...
        # Anthropic's OpenAI-compatible endpoint requires max_tokens; harmless
        # for OpenAI/Groq/Ollama. Generous enough for the JSON responses here.
        max_tokens=1500,
    )
    try:
        try:
...
def probe_model(config: Optional[ModelConfig] = None) -> bool:
    """Cheap reachability check for the status banner. Lists models rather
    than running a completion, so it never loads a local model into memory,
    and uses a short timeout so a page load never hangs on a dead endpoint."""
    config = config or ModelConfig.current()
    try:
        get_client(config, timeout=3.0).models.list()
        return True
    except Exception:
        return False
